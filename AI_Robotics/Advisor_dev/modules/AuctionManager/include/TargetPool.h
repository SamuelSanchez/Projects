...
//TODO: Separate this class among a virtual parent class and its children
//The children are selected by Advisor. The same way AuctionManager works.
class TargetPool {
   map<int,Target> targets;
   int targetsCompleted;
   map<long,bool> agents;    //Only if the agents are available can be reset for each point
   ...
   //COMBINATORIAL_AUCTION_SORTED
   //This is for sorting from all replies from cheapeast to most expensive
   vector<Bid> allBids;
 ...
 public:
   ...
   mode(COMBINATORIAL_AUCTION_PROXIMITY) {} //Let's give it a value by default
   ~TargetPool() {}

   void setMode( Auction_Mode m )
   {
      mode = m;
   }
   ...
   void updateBids()
   {
      switch( mode ){
       case COMBINATORIAL_AUCTION_PROXIMITY:
          std::cout << "Update Bids by Proximity" << std::endl;
          updateBidsProximity();
          break;
       case COMBINATORIAL_AUCTION_SORTED:
          std::cout << "Update all bids" << std::endl;
          updateAllBids();
          break;
      }
   }

   void updateBidsProximity()
   {
      ...
      //Now that all bids have been sorted for their respective replies, sort all set of Targets in ascending order
   ...
   void updateAllBids()
   {  //Sort all the bid replies
      std::cout << "Before Sorting" << std::endl;
      for( vector<Bid>::iterator  itr = allBids.begin(); itr != allBids.end(); itr++ )
          std::cout << "Bid [ " << (*itr).id << " ] - Cost [ " << (*itr).cost << " ] - Agent [ " << (*itr).robot << " ]" << std::endl;
      
      sort( allBids.begin(), allBids.end(), sortBids );
   ...
   void updateWinners()
   {
      switch( mode ){
       case COMBINATORIAL_AUCTION_PROXIMITY:
          std::cout << "Update Winners by Proximity" << std::endl;
          updateWinnersProximity();
          break;
       case COMBINATORIAL_AUCTION_SORTED:
          std::cout << "Update Winners by overall distance" << std::endl;
          updateAllWinners();
          break;
      }
   }

   void updateWinnersProximity()
   {
     ...
     // std::cout << "Agents [ " << numberOfAgents << " ] - Targets [ " << targetAgents.size() << " ]" << endl;
      for( vector<Target>::iterator itr = targetAgents.begin(); itr != targetAgents.end(); itr++ ){
          ...
          // std::cout <<  "Target[ " << (*itr).getID() << " ]" << std::endl;
           if( !(*itr).isTargetCompleted() && (*itr).hasReply() ){ //Doesn't need to check if it has a reply becuase in this vector we only keep targets with replies
   ...
   void updateAllWinners()
   {
      ...
      for( vector<Bid>::iterator itr = allBids.begin(); itr != allBids.end(); itr++ ){
           ...
           pointTaken = false;
           std::cout << "Bid [ " << (*itr).id << " ] - Point Taken [ " << pointTaken << " ]" << std::endl;
           if( !(*agents.find((*itr).robot)).second ){// If agent is available take him
              for( map<int,bidWinner>::iterator point = openBid.begin(); point != openBid.end(); point++ ){ //Let's make sure that the point is not taken
                   std::cout << "Won Bid [ " << (*point).second.bid_id << " ]" << std::endl;
                   if( (*point).second.bid_id == (*itr).id ){
                      std::cout << "Repetition bid [ " << (*itr).id << " ] - Won Bid [ " << (*point).second.bid_id << " ]" << std::endl;
                      pointTaken = true;
                      break;
                   }
              }
              if( pointTaken ) continue; //If the point is taken go next
              std::cout << "Point is valid - let's keep it" << endl;
              (*agents.find( (*itr).robot ) ).second = true;  //Setting to true means that agent cannot be reused
              std::cout << "TARGET [ " << (*itr).id << " ] - WINNER [ " << (*itr).robot << " ]" << std::endl;
              bidWinner winner( (*itr).id, (*itr).robot );
              openBid.insert( pair<int,bidWinner> ( numberOfWinners, winner ) );
              numberOfWinners++;
              counter++;
           }
   ...
   void reset()
   {
      //Reset all bids - This is when sorting all bids overall
      if( mode == COMBINATORIAL_AUCTION_SORTED ){
          allBids.clear();
      }
   ...
   void addBid( Bid bid, int targetId )
   {
      ...
      //Let's keep All the bids to only sort once
      if( mode == COMBINATORIAL_AUCTION_SORTED ){
