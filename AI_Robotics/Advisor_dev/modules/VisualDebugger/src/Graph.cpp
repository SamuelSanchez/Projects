...
Graph::Graph(Map * m, bool d, int p, bool generatePoints): proximity(p), digraph(d), map(m){
  //generateNavGraph();
  //generateEmptyGraph(); 
  if(generatePoints)
     generateAuctionGraph(); 	
}
...
void Graph::generateAuctionGraph() {
  int l = map->getLength(); 
  int h = map->getHeight(); 
  ...
  Node * n; 
  int index = 0; 
  for( int x = proximity; x < l; x += proximity ){
    for( int y = proximity; y < h; y += proximity ) {
      n = new Node(index, x, y); 
      nodes.push_back(*n); 
      index++;
    }
